...
const metaPatch = function (metadata) {
		...
		if(typeOf(metadata) !== 'object'){
				...
				if(this._wlQueryInfo.meta) {
						this._wlQueryInfo.meta = _.merge(this._wlQueryInfo.meta, metadata);
				} else {
...
function inferPopType (tokens, criteria, model) {		
		if(criteria.includes(tokens['deep'])){
				return 'deep';
		} else if(criteria.includes(tokens['incl'])){
				return 'inclusive';
		} else if(criteria && (criteria.includes(tokens['poly']) || criteria === model._polymorphicAttribKey)) {
				return 'polymorphic';
		} else {
				return 'normal';
		}
...
module.exports = function patchModelMethods  (models) {
    ...
    _.forIn(models, model =>{
        ...
        _.forEach (require('./associations-utils'), (util, key) => {
