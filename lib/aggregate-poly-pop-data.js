        ...
        if(rec.hasOwnProperty(ass)){						
            if(!_.isEmpty(rec[ass])){
								...
								if(polyOpts.usePolyAttribKey){
										...
										rec[this._polymorphicAttribKey].push(polyOpts.useModelKey ? {[this.getAssModelID(ass)]: rec[ass]} : rec[ass]);
								} else {
...
module.exports = function aggregatePolyPopData (records) {
    ...
    let _records, _aggregatePolyPopRecord = _.bind(aggregatePolyPopRecord, this._WLModel, this);
    ...
    if(_.isArray(records)){
        ...
        sails.log.warn(`${__line()} - This should be shown, there is a bug in the hook or you passed a none record to aggregatePolyPopData`);
    }
