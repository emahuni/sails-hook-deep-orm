...
module.exports = {
		...
		assureModelObj (model) {
				...
				if(typeOf(model) === 'string' && !(model = sails.models[model])){
						throw new Error(chalk`${__line()} - {red The given modelID '${model}' doesn't match any sails models!}`);
						return false;
				} else if(!model || !model.identity){
						throw new Error(chalk`${__line()} - {red The given model:\n ${model} \n\nisn't a sails model or it has no identity.}`);
						return false;
				}
		...
		getThatModelAssAliasesToThis (model) {
				...
				let asses = this.getThatModelAssesToThis(model);
				// now pluck the association aliases
				return asses && asses.map(ass => ass.alias) || [];
		},
