...
module.exports =  function populatePatch (criteria, subcriteria){
		const tokens = sails.config.deeporm.tokens;
		...
		// let subcriteriaPrint = subcriteria && inferedPopType === 'normal' ? chalk.green(u.format(subcriteria)) : (inferedPopType === 'normal' ? chalk.grey(u.format(subcriteria)) : chalk.grey.dim(u.format(subcriteria)));
		...
		if(!this._meta('polymorphic').usePolyAttribKey && !this._meta('polymorphic').useModelKey){
				...
				sails.log.warn(chalk`{keyword('orange') ${__line()} 'usePolyAttribKey' and 'useModelKey' cannot be both false; setting 'usePolyAttribKey' to true.}`)
		...
		switch (inferedPopType)
		{
				...
				//////////////////////////////////////////////////////////////////////////////////////////////////////////
				case 'deep': { // is this a deep populate
						///////////////////////////////
						// TODO: this code and the deep-populate.js code can be simplified into a loop but as depth (each deep path segment) based as opposed to records based. There's too much duplication of code. Now that I have figured out the way to do it, i now need to simplify it. It's a bit confusing to squash bugs, though it work well. Records based is where the algo first finds records and then populates the records it found using the given path (current algo). Segment based is whereby I intend to split deep path segments then run populations using those as the basis for previous, current and  next populations; this should work well in a for... of loop.
						///////////////////////////////
						...
						return this.populate(argins.criteria, argins.subcriteria).then(async (recs)=>{
								const {popRecsNextCri, singleRecPopNextCri, nextDepth, popPrevCriRecAss} = require('./deep-populate');
								...
								argins.subcriteria = typeOf(argins.nextSubcriteria) === 'array' ? argins.nextSubcriteria.shift() : argins.nextSubcriteria;
				...
				//////////////////////////////////////////////////////////////
				case 'inclusive':	{ //  is this an inclusive population
						...
						for(let cri of criterias) {
								...
								let scri = typeOf(nextSubcriteria) === 'array' ? nextSubcriteria.shift() : nextSubcriteria;
				...
				} break; // end case inclusive


				
				//////////////////////////////////////////////////////////////////////////////////////////////////////////
				case 'polymorphic': { // is this a polymorphic populate?
						...
						if(criteria.startsWith(tokens['poly'])) {
								criteria = criteria.substr(1);
						}
						...
						let all = false; 		// is this an all polymorphic population
						...
						if(all = (specificModelsIDs[0] === '' || criteria === this._WLModel._polymorphicAttribKey || _.isEmpty(specificModelsIDs))){
								specificModelsIDs = this._WLModel.getAssedModelsIDs();
						}
				...
				default: {
						...
						if(!this._meta('populationType')){
								this._meta({populationType: 'normal'});
						}
						...
						// workaround for waterline limited opinionation (this is actually nonsense, how can one know beforehand all scenarios a framework is going to be used for.
						// Since it doesn't break anything just let the choice lye with the developer and not restrict them on how they can use it):
						// This is a singular ("model") association, which means it never refers to more than _one_ associated record.
						// So passing in subcriteria (i.e. as the second argument to `.populate()`) is not supported for this association, since it generally wouldn't make any sense.
						if(this._WLModel.getAssType(criteria) !== 'collection'){
						...
						{
								case 'polymorphic': {
										let entry = _.find(this._meta('deep').queryInfo.criterias, { 'wasPolymorphic': true });
